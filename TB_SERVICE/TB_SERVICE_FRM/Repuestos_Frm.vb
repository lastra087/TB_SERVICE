...
Public Class Repuestos_Frm
    ...
    Private Sub btn_modificar_Click(sender As System.Object, e As System.EventArgs) Handles btn_modificar.Click
        If txt_id.Text <> Nothing And txt_repuesto.Text <> Nothing And txt_precio.Text <> Nothing And txt_stock.Text <> Nothing And txt_descripcion.Text <> Nothing Then
            oDs = New DataSet
            o_Repuesto = New Repuestos
            o_Repuesto.modificar_repuesto(txt_id.Text, txt_repuesto.Text, txt_stock.Text, txt_precio.Text, txt_descripcion.Text, cbo_proveedor.SelectedValue)
    ...
    Private Sub btn_buscar_Click(sender As System.Object, e As System.EventArgs) Handles btn_buscar.Click
        If txt_buscar_repuesto.Text <> Nothing Then
            ...
            Limpiar()
        Else
            MsgBox("Ingrese una repuesto", vbInformation, "Consultar datos")
        End If
    End Sub

    Private Sub dgv_repuestos_CellClick(sender As Object, e As DataGridViewCellEventArgs) Handles dgv_repuestos.CellClick
        If e.RowIndex >= 0 Then
            Dim row As DataGridViewRow = dgv_repuestos.Rows(e.RowIndex)

            txt_id.Text = row.Cells("Codigo").Value.ToString()
            txt_repuesto.Text = row.Cells("Repuesto").Value.ToString()
            txt_precio.Text = row.Cells("Precio_unitario").Value.ToString()
